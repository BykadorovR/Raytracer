...
Blur::Blur(std::vector<std::shared_ptr<Texture>> src,
  ...
  shader->add("../shaders/blur_compute.spv", VK_SHADER_STAGE_COMPUTE_BIT);
  ...
  _descriptorSet->createBlur(src, dst);
...
void Blur::drawCompute(int currentFrame, std::shared_ptr<CommandBuffer> commandBuffer) {
