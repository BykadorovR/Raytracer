...
vec3 directionalLight(int lightDirectionalNumber, vec3 fragPosition, vec3 normal, vec3 cameraPosition, 
    ...
    for (int i = 0; i < lightDirectionalNumber; i++) {
        ...
        vec3 ambientFactor = getLightDir(i).ambient * getMaterial().ambient;
        //dot product between normal and light ray
        vec3 diffuseFactor = getLightDir(i).diffuse * max(dot(lightDir, normal), 0) * getMaterial().diffuse;
        //dot product between reflected ray and light ray
        //Phong implementation
        /*vec3 specularFactor = getLightDir(i).specular * getMaterial().specular * 
        ...
        vec3 specularFactor = getLightDir(i).specular * getMaterial().specular * pow(max(dot(normal, halfwayDir), 0), max(getMaterial().shininess, 1));
        lightFactor += getLightDir(i).color * (ambientFactor + (1 - shadow) * (diffuseFactor + specularFactor)); 
    }
    return lightFactor;
}
...
vec3 pointLight(int lightPointNumber, vec3 fragPosition, vec3 normal, vec3 cameraPosition, 
    ...
    for (int i = 0; i < lightPointNumber; i++) {
        ...
        vec3 ambientFactor = getLightPoint(i).ambient * getMaterial().ambient;
        //dot product between normal and light ray
        vec3 diffuseFactor = getLightPoint(i).diffuse * max(dot(lightDir, normal), 0) * getMaterial().diffuse;
        //dot product between reflected ray and light ray
        //Phong implementation
        /*vec3 specularFactor = getLightPoint(i).specular * getMaterial().specular * 
                               pow(max(dot(normalize(reflect(-lightDir, normal)), normalize(cameraPosition - fragPosition)), 0), max(getMaterial().shininess, 1));*/
        //Blinn-Phong implementation
        vec3 viewDir = normalize(cameraPosition - fragPosition);
        vec3 halfwayDir = normalize(viewDir + lightDir);
        vec3 specularFactor = getLightPoint(i).specular * getMaterial().specular * pow(max(dot(normal, halfwayDir), 0), max(getMaterial().shininess, 1));
        lightFactor += getLightPoint(i).color * attenuation * (ambientFactor + (1 - shadow) * (diffuseFactor + specularFactor)); 
    }
